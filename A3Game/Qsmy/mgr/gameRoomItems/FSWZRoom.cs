...
namespace MuGame
{
	class FSWZRoom : BaseRoomItem
	{
		...
		public override void onStart(Variant svr) {
			...
			isOpen = true;
			data = MapModel.getInstance().getMapDta(103);
			if (data == null) {
				data = new MapData();
			}
			MapModel.getInstance().AddMapDta(103, data);
			data.OnKillNumChange = (int i) => {
				if (a3_insideui_fb.instance != null) {
					a3_insideui_fb.instance.SetKillNum(i,-1,103);
				}
			}; 
            ...
            a3_insideui_fb.ShowInUI(a3_insideui_fb.e_room.Cailiao);
            if (data != null) {
				data.cycleCount++;
			}
		}
