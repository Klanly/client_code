...
namespace MuGame
{
	class MoneyRoom : BaseRoomItem
	{
		...
		public override bool onLevel_Status_Changes(Variant msgData) {
			...
			if (msgData.ContainsKey("energy")) {
				int e = msgData["energy"];
				if (a3_insideui_fb.instance != null) {
					a3_insideui_fb.instance.SetGoldLeft(e);
				}
			}
			return true;
		}
		...
		public override bool onLevelFinish(Variant msgData) {
			...
			new timersManager().addTimer(waittime, (Action<object>)((object o) => {
				...
				al.Add(temp);
				al.Add(data.kmNum);
                //int t = (int)UnityEngine.Mathf.Max(0, PlayerModel.getInstance().money - startMoney);
                int t = a3_insideui_fb.instance.addmoney;
               // money_get = t;
                al.Add(t);
